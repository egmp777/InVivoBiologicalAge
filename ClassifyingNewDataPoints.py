...
df_ml = df_clusters
transformers = {}
for c in df_ml.select_dtypes(exclude='object').columns:
    pt = PowerTransformer()
    df_ml[c] =  pt.fit_transform(np.array(df_ml[c]).reshape(-1, 1))
    transformers[c] = pt
...
for col in categorical_features:
        ...
        new_data[col] = encoders[col] .fit_transform(new_data[col])
        # encoders[col] = le
