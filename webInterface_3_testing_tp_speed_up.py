...
with st.form("profile_data_form"):
    ...
    if st.session_state.step == 1:
        ...
        enpoint_esp = st.selectbox( 'Seleccionar que indicador desea optimizar', options=df_es['punto final'].unique().tolist(),
...
if submitted:
    ...
    if st.session_state.endpoint and st.session_state.age and st.session_state.population and st.session_state.duration:  # Ensure all fields are filled
        ...
        data = {'endpoint': [st.session_state.endpoint], 'population': [st.session_state.population], 'age': [st.session_state.age], 'duration': [st.session_state.duration]}
