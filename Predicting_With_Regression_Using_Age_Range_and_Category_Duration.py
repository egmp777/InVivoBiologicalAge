...
# file_name_clustering_data = "DataForClustering_Without_Study_Category_Duration_With_Age_Range_with_Duration.csv"

file_name_clustering_data = "DataForClustering_With_Category_Duration_With_Age_Range.csv"
...
data['HIIE_success'] = data.apply(lambda row: 1 if ((row['Mean_HIIE'] < 0 and row['Mean_HIIE'] < row['Mean_MICT']
                                                    and row['desired_effect'] == 'decrease') )| ((row['Mean_HIIE'] > 0
                                                    and row['desired_effect'] == 'increase') and row['Mean_HIIE'] >
                                                    row['Mean_MICT'])
                                                    else 0, axis=1)
data['MICT_success'] = data.apply(lambda row: 1 if ((row['Mean_MICT'] < 0 and row['Mean_HIIE'] > row['Mean_MICT']
...
#### JULY 9 2025 ####
##DEC 1 2025
data = data.drop(['desired_effect', 'Mean_HIIE', 'Mean_MICT', 'type_exercise'], axis=1)
...
def target_protocol_value(hiie, mict):
    if (hiie == 1):
...
data['target_protocol'] = data.apply(lambda x:target_protocol_value(x['HIIE_success'], x['MICT_success']), axis=1)
...
filename = 'trained_model_regression_with_age_range_and_cat_duration.joblib'
