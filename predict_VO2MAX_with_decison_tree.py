...
pandas_data['vo2max'] =  (pandas_data['Max_BPM'] / pandas_data['Resting_BPM']) * 15.3
...
# pandas_data = pandas_data.drop([ 'Max_BPM', 'Water_Intake (liters)', 'Calories_Burned', 'Resting_BPM', 'BMI', 'Fat_Percentage'],  axis = 1)
pandas_data = pandas_data.drop([  'Resting_BPM',  'Max_BPM', 'Avg_BPM', 'Experience_Level', 'Fat_Percentage', 'BMI',
                                  'Calories_Burned', 'Water_Intake (liters)', 'Session_Duration (hours)',
...
target = pandas_data['vo2max']
pandas_data = pandas_data.drop(['vo2max'], axis = 1)
...
le = LabelEncoder()
label_1 = le.fit_transform(pandas_data['Gender'])
pandas_data.drop("Gender", axis=1, inplace=True)
pandas_data["Gender"] = label_1
# label_2 = le.fit_transform(pandas_data['Workout_Type'])
# pandas_data.drop("Workout_Type", axis=1, inplace=True)
# pandas_data["Workout_Type"] = label_2
...
data_train = scaler.fit_transform(data_train) # Fit and transform on training data
data_test = scaler.transform(data_test)       # Only transform testing data
...
filename = 'trained_model_Vo2Max_linear_tree.joblib'
